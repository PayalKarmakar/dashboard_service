...
public partial class ChamberCriticalReportPage : Page
{
    ...
    private void ChamberCriticalReportPage_Unloaded(object sender, RoutedEventArgs e)
    {
        ThemeService.ThemeChanged -= ThemeService_ThemeChanged;
    }
    ...
    private async void ChamberCriticalReportPage_Loaded(object sender, RoutedEventArgs e)
    {
        ApplySciFiChrome(ThemeService.IsDarkMode);
    ...
    private async Task LoadReportAsync()
    {
        try
        {
            ...
            _rows = await _reportService.GetChamberCriticalReportAsync(
                from,
                to,
                chamberId,
            ...
            ResultCountText.Text =
                $"{_rows.Count} sensor event(s) · Total duration: {ReportDurationFormatter.Format(totalDuration)}";
        }
        catch (Exception ex)
        {
            MessageBox.Show(ex.Message, "Reports", MessageBoxButton.OK, MessageBoxImage.Error);
    ...
    {
        if (SeverityComboBox.SelectedItem is ComboBoxItem item && item.Tag is string tag)
        {
            return tag;
        }
    ...
    private void Export_Click(object sender, RoutedEventArgs e)
    {
        ...
        var dialog = new SaveFileDialog
        {
            ...
            FileName = $"chamber_critical_report_{DateTime.Now:yyyyMMdd_HHmm}.csv"
        };
        ...
        try
        {
            ...
            sb.AppendLine("Chamber Code,Chamber,Severity,Parameter,Actual Value,Threshold,Started At,Ended At,Duration,Status");
            ...
            foreach (var row in _rows)
            {
                sb.AppendLine(string.Join(",",
                    Csv(row.ChamberCode),
                    Csv(row.ChamberName),
                    Csv(row.SeverityDisplay),
                    Csv(row.Parameter),
                    Csv(row.ValueDisplay),
                    Csv(row.ThresholdDisplay),
                    Csv(row.StartedDisplay),
                    Csv(row.EndedDisplay),
                    Csv(row.DurationDisplay),
                    Csv(row.IsOngoing ? "Ongoing" : "Resolved")));
            }
